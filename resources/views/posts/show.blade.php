        ...
        <div class="container align-content-end">
            <form>
                <div class="form-floating mb-3">
                    <textarea class="form-control" placeholder="Deje su comentario aquí" id="comentario" style="height: 100px"></textarea>
            ...
            @if(is_null($comentario->respuestaA))
                <div class="card mb-3">
                    <h5 class="card-header">
                        @if($comentario->autor)
                            <a href="{{ route('users.show', $comentario->user) }}" style="text-decoration: none; color: inherit">
                                <img src="{{ $comentario->user->profile_photo_path }}" width="25" height="25" class="rounded-3"> {{ $comentario->user->name }}
                            </a>
                    ...
                    @foreach($comentario->respuesta as $respuesta)
                        @if($comentario->id == $respuesta->respuestaA)
                            <div class="card mb-3 me-4 ms-4 mb-2">
                                <h5 class="card-header">
                                    @if($respuesta->autor)
                                        <a href="{{ route('users.show', $respuesta->user) }}" style="text-decoration: none; color: inherit">
                                            <img src="{{ $respuesta->user->profile_photo_path }}" width="25" height="25" class="rounded-3"> {{ $respuesta->user->name }}
                                        </a>
